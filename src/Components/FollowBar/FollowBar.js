...
export const FollowBar = () => {
  ...
  // getting user details of current user.
  const socialUser = JSON.parse(localStorage.getItem("socialUser"));

  // getting details from users, to take updated details.
  const loggedInUser = users?.find(
    (el) => el?.username === socialUser?.username
  );

  // naviage to user profile
  ...
  return (
    <div className={`followbar-main-container ${isDarkMode && "bg-dark"}`}>
      <div
        className={`followbar-container flex  align-center ${
          isDarkMode && "bg-dark"
        }`}
      >
        ...
        {notFollowedUsers?.map((user) => {
          ...
          return (
            <div
              key={_id}
              className={`flex justify-between follow-user-card ${
                isDarkMode && "bg-dark-light  border-color-dark"
              }`}
            >
              <div className="flex followbar-card-inner align-center">
                <span
                  className="follow-user-profile cursor-pointer"
                ...
                <div
                  onClick={() => handleUserProfile(userHandler)}
                  className={`cursor-pointer ${isDarkMode && "color-white"}`}
                >
                  <h4 className="follow-user-name text-center">
                    {firstName} {lastName}
                  </h4>
                  <small
                    className={`follow-username-2 ${
              ...
              <button
                onClick={() => handleFollowUser(_id, socialToken, dataDispatch)}
                className={`follow-btn letter-spacing-1 cursor-pointer ${
        ...
        {notFollowedUsers?.length === 0 && (
          <p
            className={`letter-spacing-1 no-suggestions-text ${
              isDarkMode && "color-white"
            }`}
          >
